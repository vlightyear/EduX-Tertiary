...
public class UBAController : ControllerBase
{
        ...
        IBackgroundEmailService backgroundEmailService = null)
    {
        ...
        _ubaApiKey = _configuration["SchoolPay:ApiKey"];
        
        if (string.IsNullOrEmpty(_ubaApiKey))
        {
            _logger.LogError("UBA API Key not found in configuration. Please add 'SchoolPay:ApiKey' to appsettings.json");
            throw new InvalidOperationException("UBA API Key not configured");
        }
    }

    // POST: api/uba/VerifyStudent
    ...
    public async Task<IActionResult> VerifyStudent([FromBody] VerifyStudentRequest request)
    {
        ...
        if (!Request.Headers.TryGetValue("X-API-KEY", out var apiKey) || apiKey != _ubaApiKey)
        {
    ...
    public async Task<IActionResult> PostPayment([FromBody] PaymentRequest model)
    {
        ...
        try
        {
            ...
            {
                _logger.LogInformation("Queueing background processing for UBA payment");
                _ = Task.Run(async () =>
                {
                    ...
                    try
                    {
                        var processor = new UBAPaymentProcessor(
                            services.GetRequiredService<ApplicationDbContext>(),
                            services.GetRequiredService<ILogger<UBAPaymentProcessor>>(),
                            services.GetService<IAccountingService>(),
                            services.GetService<IBackgroundEmailService>());

                        if (payment.ApplicantId.HasValue)
                        {
                            _logger.LogInformation("Processing applicant payment via UBA");
                            await processor.ProcessApplicantPayment(payment);
                        }

                        if (payment.StudentId > 0)
                        {
                            _logger.LogInformation("Processing student payment via UBA");
                            await processor.ProcessStudentPayment(payment);
                        }
                    }
                    catch (Exception ex)
                    {
                        var logger = services.GetRequiredService<ILogger<UBAController>>();
                        logger.LogError(ex, "Background processing failed for UBA payment {PaymentId}", payment.Id);
                    }
                });
            }
        ...
        {
            _logger.LogError(ex, "Failed to save UBA payment confirmation");
...
// Payment processor specifically for UBA transactions
public class UBAPaymentProcessor
{
    private readonly ApplicationDbContext _context;
    private readonly ILogger<UBAPaymentProcessor> _logger;
    private readonly IAccountingService _accountingService;
    private readonly IBackgroundEmailService _backgroundEmailService;

    public UBAPaymentProcessor(
        ApplicationDbContext context,
        ILogger<UBAPaymentProcessor> logger,
    ...
    {
        _logger.LogInformation("Starting UBA applicant payment processing for transaction {TransactionId}",
        ...
        await strategy.ExecuteAsync(async () =>
        {
            ...
            try
            {
                ...
                _logger.LogInformation("Processing UBA application {ApplicationId} for applicant {ApplicantName}",
                ...
                if (_accountingService != null)
                {
                    try
                    {
                        _logger.LogInformation("Posting UBA registration fee to accounting system for application {ReferenceNumber}",
                        ...
                        {
                            _logger.LogWarning("Accounting system returned error for UBA application {ReferenceNumber}: {Message}",
                                application.ReferenceNumber, accountingResult.Message);
                        }
                    }
                    catch (Exception ex)
                    {
                        _logger.LogError(ex, "Accounting system error for UBA application {ReferenceNumber}",
                            application.ReferenceNumber);
                    }
                }
                ...
                {
                    _logger.LogInformation("Creating new UBA application payment record");
                    ...
                    _logger.LogInformation("Saved {Changes} database changes for UBA application payment", changes);
                }
                ...
                if (_backgroundEmailService != null)
                {
                    try
                    {
                        _logger.LogInformation("Queueing UBA application submission email");
                    ...
                    {
                        _logger.LogError(ex, "Failed to queue UBA application email");
                    }
                }

                await transactionScope.CommitAsync();
                _logger.LogInformation("Completed UBA applicant payment processing successfully");
            }
            catch (Exception ex)
            {
                await transactionScope.RollbackAsync();
                _logger.LogError(ex, "Error processing UBA applicant payment");
                throw;
            }
        });
    }
    ...
    {
        _logger.LogInformation("Starting UBA student payment processing for transaction {TransactionId}",
            transaction.MerchantTransactionId);
        ...
        await strategy.ExecuteAsync(async () =>
        {
            ...
            try
            {
                ...
                {
                    _logger.LogWarning("Financial statement already exists for UBA transaction {TransactionId}",
                        transaction.MerchantTransactionId);
                    return;
                }
                ...
                var changes = await _context.SaveChangesAsync();
                _logger.LogInformation("Saved {Changes} database changes for UBA financial records", changes);
                ...
                if (_accountingService != null)
                {
                    try
                    {
                        _logger.LogInformation("Posting UBA payment to accounting system for student {StudentId}", student.Id);
                        ...
                        if (!accountingResult.Success)
                        {
                            _logger.LogWarning("Accounting system error for UBA: {Error}", accountingResult.Message);
                        }
                    }
                    catch (Exception ex)
                    {
                        _logger.LogError(ex, "Accounting system error for UBA payment");
                    }
                }

                await transactionScope.CommitAsync();
                _logger.LogInformation("Completed UBA student payment processing successfully");
            }
            catch (Exception ex)
            {
                await transactionScope.RollbackAsync();
                _logger.LogError(ex, "Error processing UBA student payment");
                throw;
            }
        });
    }

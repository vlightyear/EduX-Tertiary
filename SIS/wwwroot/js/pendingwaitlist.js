document.addEventListener('DOMContentLoaded', function () {
    ...
    function applyFilters() {
        ...
        const table = document.getElementById('pendingWaitlist');
        ...
        if (visibleRowCount === 0) {
            ...
            if (!document.getElementById('no-results-message')) {
                ...
                noResultsRow.innerHTML = `<td colspan="8" class="px-6 py-4 text-center text-slate-500">No waitlisted applications match your filter criteria</td>`;
    ...
    function exportTableData() {
        ...
        link.setAttribute('download', 'waitlisted_applications.csv');
    ...
    function setupModalActionHandlers(applicantId) {
        ...
        const modalButtons = document.querySelectorAll('.admit-applicant, .reject-applicant');
    ...
    // Update the file link setup in the details modal population
            ...
            document.querySelectorAll(selector).forEach(button => {
                button.addEventListener('click', async function () {
                    try {
                        ...
                        if (action === 'reject') {
                            // Create and show the reject reason modal
                            const rejectReasonModal = document.createElement('div');
                            rejectReasonModal.className = 'fixed inset-0 z-50 flex items-center justify-center';
                            rejectReasonModal.innerHTML = `
                                <div class="fixed inset-0 bg-black bg-opacity-50 transition-opacity"></div>
                                <div class="relative bg-white rounded-lg shadow-xl p-6 max-w-md mx-auto">
                                    <h3 class="text-lg font-semibold mb-4 flex items-center">
                                        <i class="material-icons mr-2 text-red-600">warning</i>
                                        Reject Application
                                    </h3>
                                    <textarea
                                        id="rejectReason"
                                        class="w-full p-2 border rounded-lg mb-4 h-32"
                                        placeholder="Please provide a reason for rejection..."
                                    ></textarea>
                                    <div class="flex justify-end space-x-3">
                                        <button class="cancel-reject px-4 py-2 text-slate-600 bg-slate-100 rounded-lg hover:bg-slate-200">
                                            Cancel
                                        </button>
                                        <button class="confirm-reject px-4 py-2 text-white bg-red-600 rounded-lg hover:bg-red-700">
                                            Confirm Rejection
                                        </button>
                                    </div>
                                </div>
                            ...
                            // Handle cancel
                            rejectReasonModal.querySelector('.cancel-reject').onclick = () => {
                                rejectReasonModal.remove();
                            };

                            // Handle confirm
                            rejectReasonModal.querySelector('.confirm-reject').onclick = async () => {
                                ...
                                const confirmButton = rejectReasonModal.querySelector('.confirm-reject');
                                confirmButton.disabled = true;
                                confirmButton.innerHTML = `<i class="material-icons text-sm mr-1">hourglass_empty</i>Processing...`;
                                ...
                                try {
                                    ...
                                    const response = await fetch('/AdmissionProcess/reject', {
                                        method: 'POST',
                                        body: formData
                                    });
                                    ...
                                    showAlert({
                                        type: 'error',
                                        title: 'Error',
                                        text: error.message || 'Failed to process rejection'
                                    });
                                    confirmButton.disabled = false;
                                    confirmButton.innerHTML = 'Confirm Rejection';
                                }
                            };

                            return; // Exit early for reject action
                        }

                        // For admit action, confirm first
                        if (action === 'admit') {
                            const confirmed = await showAlert({
                                type: 'success',
                                title: 'Confirm Admission',
                                text: 'Are you sure you want to admit this applicant from the waitlist?',
                                isConfirm: true
                            });
        ...
        try {
            ...
            if (!qualifiedIds || !qualifiedIds.length) {
                showAlert({
                    ...
                    text: 'There are no qualified waitlisted applicants to admit.'
                });
                return;
            }
            ...
            const confirmed = await showAlert({
                ...
                text: `Are you sure you want to admit all ${qualifiedIds.length} qualified waitlisted applicants?`,
                isConfirm: true
            });
